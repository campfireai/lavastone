...
int main(int argc, char *argv[]) {
  ...
  std::unordered_map<std::string, size_t> loaded_num_recipes_by_author_ref =
      num_recipes_by_author_ref;
  assert(num_recipes_by_author == loaded_num_recipes_by_author_ref);

  std::map<size_t, std::vector<std::string>> authors_by_num_recipes;
  lava::Ref<std::map<size_t, std::vector<std::string>>>
      authors_by_num_recipes_ref;
  ...
  std::map<size_t, std::vector<std::string>> loaded_authors_by_num_recipes_ref =
      authors_by_num_recipes_ref;
  ...
  recipe final_persistent_elem =
      persistent_vector_ref.at(persistent_vector_ref.size() - 1);
  ...
  for (size_t i = 0; i < vecvecstring_ref.size(); i++) {
    for (size_t j = 0; j < vecvecstring_ref.at(i).size(); j++) {
      std::cout << "vecvecstring_ref.at(" << i << ").at(" << j
                << ") = " << std::string(vecvecstring_ref.at(i).at(j)) << "\n";
    }
  }

  lava::Ref<std::unordered_map<std::string, std::string>> mapstring_ref;
  mapstring_ref["pudding"] = "delicious";
  std::cout << "mapstring_ref[\"pudding\"] = "
            << std::string(mapstring_ref["pudding"]) << "\n";
  std::unordered_map<std::string, std::string> loaded_mapstring_ref =
      mapstring_ref;
  std::cout << "loaded_mapstring_ref.size() = " << loaded_mapstring_ref.size()
            << "\n";
  std::cout << "loaded_mapstring_ref[\"pudding\"] = "
            << loaded_mapstring_ref["pudding"] << "\n";

  lava::Ref<std::unordered_map<size_t, size_t>> mapint_ref;
  mapint_ref[12] = 24;
  std::cout << "mapint_ref[12] = " << mapint_ref[12] << "\n";

  lava::Ref<std::unordered_map<std::string,
                               std::unordered_map<std::string, std::string>>>
      deep_map_ref;
  deep_map_ref["abc"]["123"] = "hello world";
  std::cerr << "deep_map_ref.at(\"abc\").at(\"123\") = "
            << std::string(deep_map_ref.at("abc").at("123")) << "\n";
  std::cerr << "deep_map_ref.at(\"abc\").size() = "
            << deep_map_ref.at("abc").size() << "\n";

  lava::Ref<std::unordered_map<
      std::string,
  ...
  std::cerr << "deep_datastructure[\"abc\"][123].at(0).at(99) = "
            << std::string(deep_datastructure["abc"][123].at(0).at(99)) << "\n";
  std::cerr << "deep_datastructure[\"abc\"][123].at(0).at(99) = "
            << std::string(deep_datastructure["abc"][123].at(1).at(42)) << "\n";
  std::cerr << "deep_datastructure.at(\"abc\")(123).at(0).at(99) = "
            << std::string(deep_datastructure.at("abc").at(123).at(1).at(42))
            << "\n";
